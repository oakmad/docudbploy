...
module.exports = {
  ...
  confirmInitialized() {
    const step = elegantStatus('Checking for docudbploy.json');
    if (!this.configPath()) {
      step(false);
      throw new Error('Not initialized! Run "docudbploy init" in project root first');
    }
    step(true);
    return true;
  },
  ...
  getConfig() {
    ...
    const step = elegantStatus('Reading config in docudbploy.json');
  ...
  configPath: function search(root) {
    ...
    // walks up looking for docudbploy.json
    if (chkPath === path.sep) return false;
    const file = path.join(chkPath, 'docudbploy.json');
